...
class Addr2LineView {
	trace_editor: vscode.TextEditor;

	constructor(trace_editor: vscode.TextEditor) {
		this.trace_editor = trace_editor;
	}

	update(trace_editor: vscode.TextEditor) {
		this.trace_editor = trace_editor;
	}

	getTraceDocument() {
		return (<vscode.TextEditor>vscode.window.activeTextEditor).document;
	}
}
...
export async function activate(context: vscode.ExtensionContext) {
	...
	let addr2line = new Addr2Line(ws, "", trace_regex);
	let addr2line_view = new Addr2LineView(trace_editor);


	vscode.window.onDidChangeTextEditorSelection(changeEvent => {

		if (vscode.window.activeTextEditor === undefined || changeEvent.textEditor.document !== addr2line_view.getTraceDocument()) { return; }
			...
			vscode.workspace.openTextDocument(source_uri).then(doc => {
				vscode.window.showTextDocument(doc, vscode.ViewColumn.Beside, true).then(source_editor => {
					...
					if (trace_lines_dec !== undefined) {
						...
						addr2line_view.trace_editor.setDecorations(source_code_line, trace_lines_dec);
	...
	let disposable = vscode.commands.registerCommand('extension.examineTrace', async () => {
		...
		addr2line.workspace = ws;
		addr2line_view.trace_editor = trace_editor;

		await vscode.window.showOpenDialog(open_options).then(fileUri => {
			if (fileUri && fileUri[0]) {
				addr2line.binfile_path = fileUri[0].fsPath;
			}
		});
		...
		await vscode.window.withProgress(
			...
			async progress => {
				await addr2line.translate_lines(addr2line_view.getTraceDocument().getText().split("\n")).then(() => {
